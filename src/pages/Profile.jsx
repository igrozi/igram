...
import { ChevronLeft, MessageCircle, Star, Zap, Heart, MessageSquare, Send, Pencil, Check, X, Calendar, Trash2, Image as ImageIcon, Loader2, Reply, Pin, PinOff } from 'lucide-react';
...
const Profile = () => {
    ...
    const sortPosts = useCallback((posts) => {
        return [...posts].sort((a, b) => {
            ...
            if (!a.is_pinned && b.is_pinned) return 1;
            // Если оба закрепленные - по дате закрепления (новые сверху)
            if (a.is_pinned && b.is_pinned) {
                const dateA = a.pinned_at ? new Date(a.pinned_at) : new Date(0);
                const dateB = b.pinned_at ? new Date(b.pinned_at) : new Date(0);
                return dateB - dateA;
            }
            // Если оба обычные - по дате создания (новые сверху)
    ...
    const fetchProfileData = useCallback(async () => {
        try {
            ...
            const postsRes = await databases.listDocuments(DATABASE_ID, COLLECTION_ID_POSTS, [Query.equal('author_id', profDoc.user_id), Query.orderDesc('$createdAt'), Query.limit(50)]);
            ...
            const sortedPosts = sortPosts(postsWithComments);
    ...
    useEffect(() => {
                        ...
                        setAllPosts(prev => {
                            ...
                            if (events.some(e => e.includes('.create'))) {
                                ...
                                if (payload.author_id === profile?.user_id && !prev.find(p => p.$id === payload.$id)) {
                                    const newPost = { ...payload, comments: [] };
                                    newPosts = [...prev, newPost];
                                } else {
                                    newPosts = prev;
                                }
                            } else if (events.some(e => e.includes('.delete'))) {
                                newPosts = prev.filter(p => p.$id !== payload.$id);
                            } else if (events.some(e => e.includes('.update'))) {
                                newPosts = prev.map(p => p.$id === payload.$id ? { ...p, ...payload, comments: p.comments } : p);
                            } else {
                                newPosts = prev;
                            }
                    ...
                    if (events.some(e => e.includes(`collections.${COLLECTION_ID_COMMENTS}`))) {
                        setAllPosts(prev => prev.map(post => {
                            if (post.$id === payload.post_id) {
                                ...
                                if (events.some(e => e.includes('.create'))) {
                                    if (!updatedComments.find(c => c.$id === payload.$id)) {
                                        updatedComments.push(payload);
                                    }
                                } else if (events.some(e => e.includes('.delete'))) {
                                    updatedComments = updatedComments.filter(c => c.$id !== payload.$id);
                                }
                                return { ...post, comments: updatedComments, comments_count: updatedComments.length };
                            }
                            return post;
                        }));
                    }
        ...
        return unsubscribe;
    }, [fetchProfileData, username, profile?.user_id, sortPosts]);
    ...
    const createPost = async () => {
        ...
        try {
            ...
            setAllPosts(prev => {
                ...
                return sortPosts([...prev, { ...newPost, comments: [] }]);
            });
    ...
    const handleTogglePinPost = async (postId, isCurrentlyPinned) => {
        if (!isOwnProfile) return;
        
        try {
            const pinnedAt = !isCurrentlyPinned ? new Date().toISOString() : null;
            ...
            setAllPosts(prev => {
                const updatedPosts = prev.map(post => 
                    post.$id === postId 
                        ? { ...post, is_pinned: !isCurrentlyPinned, pinned_at: pinnedAt }
                        : post
                );
        ...
        try {
            ...
            setAllPosts(prev => prev.map(post => {
                if (post.$id === postId) {
                    ...
                    if (post.comments?.find(c => c.$id === newComment.$id)) return post;
            ...
            await databases.updateDocument(DATABASE_ID, COLLECTION_ID_POSTS, postId, { comments_count: (postToUpdate?.comments_count || 0) + 1 });
    ...
    const handleDeleteComment = async (postId, commentId) => {
        ...
        try {
            ...
            await databases.updateDocument(DATABASE_ID, COLLECTION_ID_POSTS, postId, { comments_count: (post.comments_count || 0) - 1 });
        }
    ...
    const RatingStars = ({ vote, onRate, disabled }) => {
        return (
            <div className="flex justify-center gap-1">
                {[1, 2, 3, 4, 5].map(star => (
                    <button key={star} disabled={disabled} onMouseEnter={() => !disabled && setHoverRating(star)} onMouseLeave={() => setHoverRating(0)} onClick={(e) => { e.preventDefault(); onRate(star); }} className={`transition-all cursor-pointer ${disabled ? 'cursor-default opacity-80' : 'hover:scale-125 active:scale-95 cursor-pointer'}`}>
    ...
    const PostComponent = ({ post }) => {
        ...
        return (
            <motion.div 
                layout
                initial={isInitialLoad ? false : { opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                transition={{ duration: 0.2 }}
                className={`p-8 bg-gray-50 dark:bg-slate-800/20 border-2 ${isPinned ? 'border-yellow-500/60 bg-yellow-500/5' : 'border-gray-200 dark:border-slate-800/60'} rounded-[3rem] transition-all group shadow-sm hover:shadow-md`}
            >
                {isPinned && (
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {currentUser && post.author_id === currentUser.$id && (
                                    <>
                                        ...
                                        <button onClick={() => handleDeletePost(post.$id)} className="text-red-500 hover:text-red-400 transition-colors p-1 cursor-pointer" title="Удалить пост"><Trash2 size={16} /></button>
                                    </>
                                )}
                            </div>
                        </div>
                ...
                <div className="flex items-center gap-6 pt-4 border-t border-gray-200 dark:border-slate-700/50">             
                    <button onClick={(e) => handleLikePost(post.$id, post.likes || [], e)} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-bold cursor-pointer ${post.likes?.includes(currentUser?.$id) ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20 shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-red-500 hover:bg-gray-200 dark:hover:bg-slate-800'}`}>
                    ...
                    <button onClick={() => setShowComments(prev => ({...prev, [post.$id]: !prev[post.$id]}))} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-bold cursor-pointer ${showComments[post.$id] ? 'bg-blue-500/10 text-blue-500 dark:text-blue-400 hover:bg-blue-500/20 shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-blue-500 hover:bg-gray-200 dark:hover:bg-slate-800'}`}>
                        ...
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700/50 overflow-hidden">
                            ...
                            {replyingTo && replyingTo.postId === post.$id && (
                                <div className="mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800 flex items-center justify-between">
                                    ...
                                    <button onClick={() => setReplyingTo(null)} className="text-gray-500 hover:text-gray-700 cursor-pointer">
                                        <X size={14} />
                                    </button>
                                </div>
                            )}
                                ...
                                <div className="mb-6 flex gap-3">
                                    ...
                                    <div className="flex-1 flex gap-2">
                                        ...
                                        <button 
                                            ...
                                            className="px-6 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-md cursor-pointer"
                                        >
                                            <Send size={16} />
                                        </button>
                                    </div>
                                                    ...
                                                    <div className="flex justify-between items-start mb-1">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {currentUser && (comment.author_id === currentUser.$id || isOwnProfile) && (
                                                                <button onClick={() => handleDeleteComment(post.$id, comment.$id)} className="text-gray-400 dark:text-slate-500 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer" title="Удалить комментарий"><Trash2 size={14} /></button>
                                                            )}
        ...
        <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'dark bg-[#020617] text-white' : 'bg-gray-100 text-gray-900'}`}>
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 py-10">
                <header className="flex justify-between items-center mb-12">
                    <button onClick={() => navigate(-1)} className="p-4 bg-white/80 dark:bg-slate-900/50 border-2 border-gray-300 dark:border-slate-800 rounded-3xl hover:border-indigo-500 transition-all shadow-xl hover:scale-105 cursor-pointer"><ChevronLeft className="dark:text-white text-gray-600" /></button>
                    <div className="bg-white/80 dark:bg-slate-900/50 border-2 border-gray-300 dark:border-slate-800 px-6 py-3 rounded-3xl flex items-center gap-3 shadow-md">
                        <div className={`w-2 h-2 rounded-full ${checkIsOnline(profile) ? 'bg-indigo-500 animate-pulse' : 'bg-gray-400 dark:bg-slate-600'}`} />
                        <span className={`text-[10px] font-black tracking-[0.3em] uppercase opacity-60 ${checkIsOnline(profile) ? 'text-indigo-600 dark:text-indigo-500' : 'text-gray-600 dark:text-gray-400'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    <div className="lg:col-span-4">
                        <div className="bg-white/90 dark:bg-slate-900/80 border-4 border-gray-200 dark:border-slate-800 rounded-[3.5rem] p-10 text-center shadow-2xl sticky top-10">
                            ...
                            <p className="text-indigo-600 dark:text-indigo-400 font-bold text-sm tracking-widest mb-8 opacity-80">@{profile?.username}</p>
                            ...
                            <div className="mt-8 p-6 bg-gray-100/80 dark:bg-slate-950/50 rounded-3xl border-2 border-gray-200 dark:border-slate-800 shadow-inner">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    {isOwnProfile && !editingBio && (
                                        <button onClick={() => setEditingBio(true)} className="text-indigo-500 dark:text-indigo-400 hover:text-indigo-400 dark:hover:text-indigo-300 transition-colors cursor-pointer"><Pencil size={16} /></button>
                                    )}
                                </div>
                                {editingBio ? (
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={handleUpdateBio} className="flex-1 text-white bg-indigo-600 hover:bg-indigo-700 py-2 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors cursor-pointer"><Check size={24} /></button>
                                            <button onClick={() => { setEditingBio(false); setNewBio(profile?.bio || ""); }} className="flex-1 text-white bg-gray dark:bg-slate-800/50 hover:bg-gray-900 py-2 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors cursor-pointer"><X size={24} /></button>
                                        </div>
                                    </div>
                                ) : (
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        ...
                        <div className="bg-white/90 dark:bg-slate-900/80 border-4 border-gray-200 dark:border-slate-800 rounded-[4rem] p-10 shadow-2xl">
                                ...
                                <div className="mb-10 p-6 bg-gray-50 dark:bg-slate-800/40 rounded-[2.5rem] border-2 border-gray-200 dark:border-slate-700/50 shadow-inner">
                                    ...
                                    <AnimatePresence>
                                        {imagePreview && (
                                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="relative w-full mb-6 rounded-3xl overflow-hidden border-2 border-gray-300 dark:border-slate-700 shadow-md">
                                                ...
                                                <button onClick={clearImageSelection} className="absolute top-4 right-4 p-2 bg-black/60 rounded-xl text-white hover:bg-red-500 transition-colors backdrop-blur-md cursor-pointer" title="Удалить фото"><X size={18}/></button>
                                            </motion.div>
                                        )}
                                    </AnimatePresence>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button onClick={() => fileInputRef.current?.click()} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 flex items-center gap-2 font-bold transition-colors cursor-pointer"><ImageIcon size={18} /> Фото</button>
